...
export function MobileNav({ items }: { items: NavItem[] }) {
  ...
  return <div className="mobile-nav">
    ...
    <AnimatedDialog open={open} onDismiss={() => setOpen(false)} label="Site navigation" className="mobile-menu-panel" id="mobile-menu">
      ...
      <nav aria-label="Mobile navigation">{items.map((item) => <Link data-dialog-item key={item.href} href={`/${item.href}`} onClick={() => setOpen(false)}><span>{item.index}</span><strong>{item.label}</strong><small aria-hidden="true">↗</small></Link>)}</nav>
